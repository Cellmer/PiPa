...
class EncoderDecoder(BaseSegmentor):
    ...
    def encode_decode(self, img, img_metas):
        ...
        out, fuse_x = self._decode_head_forward_test(x, img_metas)
        out = resize(
            input=out,
            size=img.shape[2:],
            mode='bilinear',
    ...
    def _decode_head_forward_train(self,
        ...
        training."""
        losses = dict()
        loss_decode = self.decode_head.forward_train(x, feature_outputs, stage, img_metas,
                                                     gt_semantic_seg,
                                                     self.train_cfg,
                                                     seg_weight)
    ...
    def forward_train(self,
        ...
        feature_outputs = {'seg': out, 'embed': emb}
        losses = dict()
        if return_feat:
            losses['features'] = x
        loss_decode = self._decode_head_forward_train(x, feature_outputs, stage, img_metas,
                                                      gt_semantic_seg,
                                                      seg_weight)
